...
def hash_update(delta: dict) -> str:
    ...
    # NIST SHA3-256 (hashlib.sha3_256) uses different padding than Ethereum's
    # Keccak256 and produces different digests for the same input, so it is not
    # a valid substitute here. pycryptodome's keccak(digest_bits=256) matches
    # the Keccak256 used by Ethereum/Solidity's keccak256().
    from Crypto.Hash import keccak
    hash_obj = keccak.new(data=tensor_bytes, digest_bits=256)
