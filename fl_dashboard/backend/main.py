...
import os
PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent  # Go up from fl_dashboard/backend to repo root
...
def simulate_fl_round():
    """
    Synthetic demo round generator — does NOT run real federated training.

    Nudges hardcoded baseline AUROCs by a small random amount, recomputes the
    real FedProxFairness weight formula on those synthetic numbers, and appends
    a block to the audit chain. Use this to demo the dashboard UI without
    waiting for a live FL run. For real results, run `fl_server_enhanced.py`
    plus the hospital clients and read /api/rounds and /api/blockchain instead
    — those reflect whatever is actually in fl_results/, real or synthetic.
    """
    ...
    return {
        ...
        "timestamp": timestamp,
        "message": f"Round {current_round} simulated (synthetic demo data, not live training). Block {len(blockchain)-1} committed."
    }
