...
class FedProxFairness(Strategy):
    ...
    def aggregate_fit(
        ...
        # Save the aggregated global model so it can actually be loaded later
        # (e.g. for real personalization/fine-tuning) instead of only logging
        # metrics. Every hospital shares the same UnifiedFLModel architecture,
        # so the ndarray list maps 1:1 onto a fresh instance's state_dict keys.
        checkpoint_dir = self.log_dir / "checkpoints"
        checkpoint_dir.mkdir(parents=True, exist_ok=True)
        arrays = fl.common.parameters_to_ndarrays(aggregated_params)
        state_dict = {
            k: torch.tensor(v)
            for k, v in zip(UnifiedFLModel().state_dict().keys(), arrays)
        }
        torch.save(state_dict, checkpoint_dir / f"global_model_round_{server_round}.pth")
        torch.save(state_dict, checkpoint_dir / "global_model_latest.pth")
        print(f"+ Global model checkpoint saved: {checkpoint_dir / 'global_model_latest.pth'}")
